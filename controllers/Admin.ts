...
export const download = async (req: Request, res: Response) => {
  const setting = await Download.findOne();
  res.render('download', { title: 'download', setting, message: req.flash('error') })
}
...
export const postSetting = async (req: Request, res: Response) => {
  try {
    ...
    const settingData = {
      ...
      generateM3U8Segments: Boolean(generateM3U8Segments)
    };
